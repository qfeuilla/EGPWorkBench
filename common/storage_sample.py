...
class Storage():
    ...
    def __init__(self, obs_shape, hidden_state_size, num_steps, num_envs, num_timesteps, device):
    ...
    def reset(self):
        self.obs_batch = torch.zeros(self.num_timesteps, self.num_steps+1, self.num_envs, *self.obs_shape)
        self.hidden_states_batch = torch.zeros(self.num_timesteps, self.num_steps+1, self.num_envs, self.hidden_state_size)
        self.act_batch = torch.zeros(self.num_timesteps, self.num_steps, self.num_envs)
        self.rew_batch = torch.zeros(self.num_timesteps, self.num_steps, self.num_envs)
        self.done_batch = torch.zeros(self.num_timesteps, self.num_steps, self.num_envs)
        self.log_prob_act_batch = torch.zeros(self.num_timesteps, self.num_steps, self.num_envs)
        self.value_batch = torch.zeros(self.num_timesteps, self.num_steps+1, self.num_envs)
        self.target_idxs = torch.zeros(self.num_timesteps, self.num_envs, 3, 8, 8)
        self.return_batch = torch.zeros(self.num_timesteps, self.num_steps, self.num_envs)
        self.adv_batch = torch.zeros(self.num_timesteps, self.num_steps, self.num_envs)
        self.info_batch = deque(maxlen=self.num_steps)
        self.step = 0
        self.timestep = 0

    def store(self, obs, hidden_state, act, rew, done, info, log_prob_act, value, target):
        self.obs_batch[self.timestep, self.step] = torch.from_numpy(obs.copy())
        self.hidden_states_batch[self.timestep, self.step] = torch.from_numpy(hidden_state.copy())
        self.act_batch[self.timestep, self.step] = torch.from_numpy(act.copy())
        self.rew_batch[self.timestep, self.step] = torch.from_numpy(rew.copy())
        self.done_batch[self.timestep, self.step] = torch.from_numpy(done.copy())
        self.log_prob_act_batch[self.timestep, self.step] = torch.from_numpy(log_prob_act.copy())
        self.value_batch[self.timestep, self.step] = torch.from_numpy(value.copy())
        # self.target_idxs[self.timestep, self.step] = target.clone()
        self.info_batch.append(info)

        self.step = (self.step + 1) % self.num_steps

    def store_last(self, last_obs, last_hidden_state, last_value, last_target):
        self.obs_batch[self.timestep, -1] = torch.from_numpy(last_obs.copy())
        self.hidden_states_batch[self.timestep, -1] = torch.from_numpy(last_hidden_state.copy())
        self.value_batch[self.timestep, -1] = torch.from_numpy(last_value.copy())
        self.target_idxs[self.timestep] = last_target.clone()
